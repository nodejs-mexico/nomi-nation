...
var print_app_logs = function (appname, nodeapi) {
  console.log(util.inspect(arguments, true, 5))
  if (typeof nodeapi == 'undefined') nodeapi = new node(config.username, config.password, config.apihost, config.apisecure);
...
module.exports = {
    ...
    start: function(args) {
        ...
        nodeapi.app_start(appname, function (err, data) {
            ...
            // log.info('sleeping for 5 seconds...'.bold.green);
            // setTimeout(function () {
            //  print_app_logs(appname, nodeapi);
            // }, 5000);
            /*
            nodeapi.app_logs(appname, function (err, data) {
                if (err) {
    ...
    init: function(args) {
        ...
        nodeapi.app_info(appname, function (err, data) {
            ...
            var child = exec('mkdir ' + folder + '; cd ' + folder + '; git init; git remote add origin ' + data.gitrepo, function (error, stdout, stderr) {
                ...
                var child2 = exec(cmd, function (error, stdout, stderr) {
                    ...
                    nodeapi.app_start(appname, function (err, data) {
                        ...
                        if (data.status == "success") {
                            ...
                            log.plain('     cd ./' + folder);
                            log.plain('     curl http://' + folder + '.cloudno.de/');
